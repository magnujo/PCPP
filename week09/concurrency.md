# Concurrency
#### by Jørgen Staunstrup, jst@itu.dk, draft version Oct 16, 2020
...
Concurrency programming has a number of additional challenges compared to sequential programs, first and foremost
...
The greatest challenge in concurrency programming is getting the streams to reliably interact in the face of 
...
For example, the first programming languages used the term *process* to denote a single independent stream of statements/instructions. One may also
find terms like task, ...
...
always the key challenge. Finding the right abstract program structure is often the key challenge.
...
stream of statements a stream. In addition to the threads, Java have concepts that allow threads to communicate
with each other.
...
on the same processor (core). Since each of the streams is idle (waiting) most of the time, none of them will "nice" that other streams
...
the introduction of a number of programming concepts allowing streams to interact. As with the terminology for the
stream concept itself, the terminology used for coordination is confusing and inconsistent. 
...
physically share some or all of their memory. In other cases, they may be connected with a network, where information can be exchanged.
...
<img src="img/RxStreams.jpg" alt="drawing" style="width:50%;" />

The stopwatch has three buttons (Start, Stop and Reset) and a textfield for showing the running time. Following the principles outlined above the Stopwatch
can be programmed with three streams (one for each of the buttons), a stream for writing the textfield.  Finally,
...
You may find a complete Java version of the Stopwatch here:  <link>

## Coordination using message passing

When using message passing, it is not possible to share objects (memory). The state of the Stopwatch is kept locally in the textField stream.
